# Универсальный запускатор бота.
#
# Нужен на случай, когда у пользователя в терминале нет активированного venv
# (например, открыл новое окно — а там conda base активна, или вообще
...
def main():
    # 1. Если виртуального окружения ещё нет — создадим
    ...
    # 2. Поставим/обновим зависимости (если уже стоят — pip отработает почти мгновенно)
    print("Проверяю зависимости из requirements.txt ...")
    subprocess.check_call([py, "-m", "pip", "install", "-q", "-r", REQS])

    # 3. Запускаем bot.py внутри venv. execv заменяет текущий процесс,
